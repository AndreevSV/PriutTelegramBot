    ...
    public void startingChat(OwnerDog owner) {
        ...
        if (chatByOwner.isPresent()) {
            ...
            SendMessage messageToOwner = new SendMessage(ownerChatId, """
                    ...
                    Введите свой вопрос. Для прекращения нажмите *Завершить* или введите команду */cats_close*
                    """)
...
//                volunteerOptional.get().setChatsOpened(1);
                ...
                SendMessage messageToVolunteer = new SendMessage(volunteerChatId, """
                        ...
                        По завершению чата нажмите *Завершить* или введите команду */dogs_close*
                        """)
                        .parseMode(ParseMode.Markdown)
                        .replyMarkup(inlineKeyboardMarkup);
                ...
                SendMessage messageToOwner = new SendMessage(ownerChatId, """
                        ...
                        Пожалуйста, дождитесь ответа волонтера, либо введите команду */dogs_close* для завершения.
                        """)
                        .parseMode(ParseMode.Markdown);
    ...
    public void executeReplyButtonCommandForVolunteer(Update update) {
        ...
        OwnerDog volunteer = ownersDogsHandler.returnVolunteerFromUpdate(update);
        ...
        if (volunteer != null) {
            ...
            if (chatByVolunteer.isPresent()) {
                ...
                SendMessage messageToOwner = new SendMessage(ownerChatId, """
                        ...
                        Для завершения консультации отправьте команду */dogs_close*
                        """)
                        .parseMode(ParseMode.Markdown);
                ...
                SendMessage sendMessage = new SendMessage(volunteerChatId, """
                        ...
                        Для завершения консультации введите команду */dogs_close*
                        """)
                        .parseMode(ParseMode.Markdown);
    ...
    public void executeCloseButtonCommand(Update update) {
        ...
        OwnerDog owner = ownersDogsHandler.returnOwnerFromUpdate(update);
