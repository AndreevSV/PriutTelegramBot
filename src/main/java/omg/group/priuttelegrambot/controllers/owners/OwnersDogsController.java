...
public class OwnersDogsController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Клиент успешно добавлен в клиентскую базу",
                    content = {@Content(mediaType = "application/json", schema = @Schema(implementation = OwnerDog.class))}),
    ...
    @GetMapping
    @Operation(summary = "Получение данных о клиенте приюта кошек",
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Информация о клиенте по заданным параметрам получена",
                    content = {@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = OwnerDog.class))}),
            @ApiResponse(responseCode = "400", description = "Параметры запроса отсутствуют или имеют некорректный формат"),
            @ApiResponse(responseCode = "404", description = "Клиент с такими данными отсутствует в клиентской базе приюта"),
            @ApiResponse(responseCode = "500", description = "Произошла ошибка, не зависящая от вызывающей стороны")
    })
    public ResponseEntity<List<OwnerDogDto>> findByIdUsernameSurnameTelephone(
        ...
        if (telephone != null && !telephone.isBlank()) {
            return ResponseEntity.ok(ownersDogsService.findByPhoneNumber(telephone));
        }
    ...
    public ResponseEntity<HttpStatus> setAnimalsToOwnerAndSetStartOfProbationPeriod(
            ...
            @RequestBody List<Long> catsIds, LocalDate dateStart) {
        ownersDogsService.setPetsToOwnerAndDateIncome(ownerId, catsIds, dateStart);
        return ResponseEntity.ok().build();
    }
